...
def render(c, prts):
    ...
    if c.get("tabular"):
        out += [
        "> Дело табличное: имя, отчество, фамилия и возраст стоят в разных "
        "клетках, и знаки на строку тут ничего не говорят. Смотрите на "
        "уверенность.",
        "",]
    out += [
        "> **Это не транскрипция, а распознавание.** Модель ошибается в знаках, "
        "путает близкие буквы и охотно дописывает правдоподобное там, где на "
        "листе другое. Текст годится, чтобы **найти лист** по корню слова; "
        "не годится, чтобы цитировать. Найдя нужное место, смотрите образ.",
        "",
        "Как искать: по корню с заменой гласных — модель пишет «Тронка» вместо "
        "«Тренка», «пинежени» вместо «пенежан». Печатайте все попадания, а не "
        "первое.",
        "",
